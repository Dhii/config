 ...
 * @since [*next-version*]
 */
class StringSplitCapableTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Config\StringSplitCapableTrait';
    ...
    /**
     * Tests that `_stringSplit()` works as expected.
     *
     * @since [*next-version*]
     */
    public function testStringSplit()
    {
        $strings = [uniqid('string'), uniqid('string'), uniqid('string')];
        $separator = uniqid('separator');
        $input = implode($separator, $strings);
        ...
        $result = $_subject->_stringSplit($input, $separator);
        $this->assertEquals($strings, $result, 'The result of string splitting is wrong');
    }
}
