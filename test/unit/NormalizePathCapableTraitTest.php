...
use Dhii\Config\NormalizePathCapableTrait as TestSubject;
use Xpmock\TestCase;
 ...
 * @since [*next-version*]
 */
class NormalizePathCapableTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Config\NormalizePathCapableTrait';
    ...
    /**
     * Tests whether `_normalizePath()` works as expected when path is an array (list).
     *
     * @since [*next-version*]
     */
    public function testNormalizePathArray()
    {
        $path = [uniqid('segment'), uniqid('segment'), uniqid('segment')];
        ...
        $result = $_subject->_normalizePath($path, $separator);
        $this->assertEquals($path, $result, 'Wrong result of array path normalization');
    }

    /**
     * Tests whether `_normalizePath()` works as expected when path is a string.
     *
     * @since [*next-version*]
     */
    public function testNormalizePathString()
    {
        $segments = [uniqid('segment'), uniqid('segment'), uniqid('segment')];
        $separator = uniqid('separator');
        ...
        $result = $_subject->_normalizePath($path, $separator);
        $this->assertEquals($segments, $result, 'Wrong result of string path normalization');
    }
}
