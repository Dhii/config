...
use Dhii\Config\GetDataCapableByPathTrait as TestSubject;
use Xpmock\TestCase;
 ...
 * @since [*next-version*]
 */
class GetDataCapableByPathTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Config\GetDataCapableByPathTrait';
    ...
    public function testGetData()
    {
        ...
        $data = (object) [$key => $val];
        $path = [$key];
        $subject = $this->createInstance(['_getPathSegmentSeparator', '_normalizePath', '_getDataStore', '_containerGetPath']);
        ...
        $subject->expects($this->exactly(1))
            ->method('_containerGetPath')
            ->with($data, $path)
            ->will($this->returnValue($data->{$key}));

        $result = $_subject->_getData($key);
        $this->assertEquals($val, $result, 'Incorrectly value returned');
    }
}
